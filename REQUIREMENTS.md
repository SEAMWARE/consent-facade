...
**⚠ BLOCKED — provider-keyed participant SDs are incompatible with the current consent-manager (verified live, DSC keying attempted and reverted).** Unlike catalog/contract ids (dereferenced as **direct URLs**, where `~` is a path-safe RFC 3986 unreserved char, and never base64-encoded), a **participant** SD is **base64-encoded by the consent-manager and spliced raw into a path**: `getPrivacyNoticesFromContractsBetweenParties` builds `` `${CONTRACT_SERVICE_BASE_URL}/bilaterals/for/${dataProviderID}?hasSigned=true` `` (`utils/contracts.js:115`) with the **standard**-base64 id and **no `encodeURIComponent`**. Standard base64 can contain `+` and `/`, which are not path-safe; the facade (Micronaut) returns `404` on a raw `+`. The **bare** SD's base64 happened to avoid `+`/`/`; the longer **keyed** SD (`default~urn:ngsi-ld:organization:…`) base64-encodes **with a `+`** → facade `404` → `getPrivacyNotices` `500`. The facade itself is correct: the same `/bilaterals/for` with the id **percent-encoded** returns the keyed contract (`200`, verified). So provider-keyed participant routing needs a **consent-manager fix** (percent-encode the id, or emit base64url) or a URL-safe participant-id scheme the CM round-trips — out of our control. **The DSC therefore keeps bare participant SDs** (register Job + `consent_grant.sh`); the facade's keyed catalog/contract ids and its Phase-4 participant *routing* are unaffected.

**Consumer-org modelling (resolved):** the `providerKey` in a participant SD URL names *which backend holds that org record*, not "this participant is that provider". A consumer's org is registered in the provider's `tm-forum-api` (as an ordering party), so the consumer's SD URL is keyed with the **provider's** key — both parties' SD URLs route to the same backend, which is exactly what the receipt build (which dereferences both) needs. (Moot while participant SDs stay bare per the blocker above.)
...
- `PersistentProviderRegistry` (`@Requires persistent=true`): Micronaut Data JDBC over a `provider(provider_key, tmforum_base_url, self_description)` table (`ProviderEntity` + `ProviderRepository`), on **PostgreSQL** (`@JdbcRepository(dialect = POSTGRES)`, `org.postgresql` driver). The schema is created by a **Flyway** migration (`db/migration/V1__create_provider_table.sql`) rather than `schema-generate`, so it is idempotent across restarts (real persistence). On first start (empty table) it **seeds from the same `facade.providers.*` config** the static registry reads, so a deployment can migrate config → database without losing providers. Reads are served from an in-memory cache refreshed on every write; the default-provider invariant still holds.
...
- **Deployment note:** persistent mode needs a `datasources.default` pointing at PostgreSQL (`dialect: POSTGRES`) and `flyway.datasources.default.enabled: true`. Tests run the same Flyway migration against **H2 in PostgreSQL-compatibility mode** (`MODE=PostgreSQL;DATABASE_TO_LOWER=TRUE`), so no database container is needed for CI. H2 is a test-only dependency; production ships the PostgreSQL driver.
